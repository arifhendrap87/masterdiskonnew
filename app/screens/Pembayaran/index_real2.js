  ...
  function renderItemFeaturedDestination(item) {
    ...
    return (
      <CardCustom
        ...
        onPress={() => navigation.navigate("Hotel", { id_city: item.id_city })}
  ...
  function renderItemRoomPromo(item) {
    ...
    return (
      <CardCustom
        ...
        propDesc={{ text: "" }}
        propPrice={{
          price: priceSplitter(item.product_price),
          startFrom: true,
        }}
        propPriceCoret={{
          price: priceSplitter(item.product_price_correct),
          discount: priceSplitter(item.product_discount),
          discountView: true,
        }}
  ...
  function renderItemEvent(item) {
    ...
    return (
      <CardCustom
        propImage={{ height: wp("40%"), url: item.img_featured_url }}
        propInframe={{
          top: this.convertDateDM(item.product_time),
          bottom: item.product_cat,
        }}
        propTitle={{ text: item.product_name }}
        propDesc={{ text: "" }}
        propPrice={{
    ...
    try {
      if (login != false) {
        ...
        const response = await fetch(url, requestOptions)
          .then((response) => response.json())
          .then((result) => {
            console.log("detailorder", JSON.stringify(result));
          })
          .catch((error) => {
            // this.dropdown.alertWithType("info", "Info", JSON.stringify(error));
          });
