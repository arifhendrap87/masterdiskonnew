...
export default function Review(props) {
    ...
    const [dataReview, setDataReview] = useState(DataReview);
    ...
    function getProduct() {
        ...
        let url = baseUrl + 'product/review?id_user=' + userSession.id_user;
    ...
    if (notification.length == 0) {
        ...
        content = <FlatList
            ...
            renderItem={({ item, index }) => (
                <CardCustomReview
                    image={item.image}
                    txtLeftTitle={item.title_review}
                    txtContent={item.content_review}
                    txtRight={item.content_review}
