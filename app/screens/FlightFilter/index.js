...
export default class FlightFilter extends Component {
    ...
    componentDidMount() {
        const { filterParam, airline, transits, facilitiess, departureTime, arrivalTime } = this.state;

        //filtermaskapai
        if (filterParam?.filter_airline_code) {

            airline.forEach(x => {
                x.selected = this.checkFilter(filterParam.filter_airline_code, x.id) == 0 ? false : true;
            })
        ...
        //filtertransit
        if (filterParam?.filter_transit) {
            transits.forEach(x => {
                x.selected = this.checkFilter(filterParam.filter_transit, x.id) == 0 ? false : true;
            })
            this.setState({ transits: transits });
        ...
        //filterbaggage
        if (filterParam?.filter_baggage) {
            if (filterParam.filter_baggage == 1) {
                facilitiess[0].selected = true;
            } else {
                facilitiess[0].selected = false;
            }
            this.setState({ facilitiess: facilitiess });
        ...
        //filtermeal
        if (filterParam?.filter_meal) {
            if (filterParam.filter_meal == 1) {
                facilitiess[1].selected = true;
            } else {
                facilitiess[1].selected = false;
            }
            this.setState({ facilitiess: facilitiess });
        ...
        //filterdeparturetime
        if (filterParam?.filter_departure_time) {
            if (filterParam.filter_departure_time[0].min == 0 && filterParam.filter_departure_time[0].max == 6) {
                departureTime[0].selected = true;
                departureTime[1].selected = false;
                departureTime[2].selected = false;
                departureTime[3].selected = false;
                departureTime[4].selected = false;
            } else if (filterParam.filter_departure_time[0].min == 6 && filterParam.filter_departure_time[0].max == 12) {
                departureTime[0].selected = false;
                departureTime[1].selected = true;
                departureTime[2].selected = false;
                departureTime[3].selected = false;
                departureTime[4].selected = false;
            } else if (filterParam.filter_departure_time[0].min == 12 && filterParam.filter_departure_time[0].max == 18) {
                departureTime[0].selected = false;
                departureTime[1].selected = false;
                departureTime[2].selected = true;
                departureTime[3].selected = false;
                departureTime[4].selected = false;
            } else if (filterParam.filter_departure_time[0].min == 18 && filterParam.filter_departure_time[0].max == 24) {
                departureTime[0].selected = false;
                departureTime[1].selected = false;
                departureTime[2].selected = false;
                departureTime[3].selected = true;
                departureTime[4].selected = false;
            } else if (filterParam.filter_departure_time[0].min == 0 && filterParam.filter_departure_time[0].max == 24) {
                departureTime[0].selected = false;
                departureTime[1].selected = false;
                departureTime[2].selected = false;
                departureTime[3].selected = false;
                departureTime[4].selected = true;
            }
            this.setState({ departureTime: departureTime });
        ...
        //filterarrivalTime
        if (filterParam?.filter_arrival_time) {
            if (filterParam.filter_arrival_time[0].min == 0 && filterParam.filter_arrival_time[0].max == 6) {
                arrivalTime[0].selected = true;
                arrivalTime[1].selected = false;
                arrivalTime[2].selected = false;
                arrivalTime[3].selected = false;
                arrivalTime[4].selected = false;
            } else if (filterParam.filter_arrival_time[0].min == 6 && filterParam.filter_arrival_time[0].max == 12) {
                arrivalTime[0].selected = false;
                arrivalTime[1].selected = true;
                arrivalTime[2].selected = false;
                arrivalTime[3].selected = false;
                arrivalTime[4].selected = false;
            } else if (filterParam.filter_arrival_time[0].min == 12 && filterParam.filter_arrival_time[0].max == 18) {
                arrivalTime[0].selected = false;
                arrivalTime[1].selected = false;
                arrivalTime[2].selected = true;
                arrivalTime[3].selected = false;
                arrivalTime[4].selected = false;
            } else if (filterParam.filter_arrival_time[0].min == 18 && filterParam.filter_arrival_time[0].max == 24) {
                arrivalTime[0].selected = false;
                arrivalTime[1].selected = false;
                arrivalTime[2].selected = false;
                arrivalTime[3].selected = true;
                arrivalTime[4].selected = false;
            } else if (filterParam.filter_arrival_time[0].min == 0 && filterParam.filter_arrival_time[0].max == 24) {
                arrivalTime[0].selected = false;
                arrivalTime[1].selected = false;
                arrivalTime[2].selected = false;
                arrivalTime[3].selected = false;
                arrivalTime[4].selected = true;
            }
            this.setState({ arrivalTime: arrivalTime });
        ...
        //filterarrivalTime
        if (filterParam?.filter_price) {
            this.setState({ priceBegin: filterParam.filter_price[0] });
            this.setState({ priceEnd: filterParam.filter_price[1] });
                                ...
                                <View style={{ flexDirection: 'row' }}>
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <TextValidator
                                            ...
                                            value={this.state.priceBegin.toString()}
                                            onChangeText={(priceBegin) => {
                                                this.setState({ priceBegin: priceBegin.replace(/[^0-9]/g, '') })
                                    ...
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <TextValidator
                                            ...
                                            type="text"
                                            //keyboardType="email-address"
                                            value={this.state.priceEnd.toString()}
                                            onChangeText={(priceEnd) => {
                                                this.setState({ priceEnd: priceEnd.replace(/[^0-9]/g, '') })
