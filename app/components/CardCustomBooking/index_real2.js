  ...
  render() {
    ...
    if (item.product == "Flight") {
      desc = (
        ...
        >
          {item.detail[0].type}, {item.detail[0].order_detail[0].cabin_name}{" "}
          Class, {item.detail[0].pax.length} orang
        </Text>
      );
      ...
      desc = (
        ...
        >
          {item.detail[0].order.country}, {item.detail[0].order.duration} hari,{" "}
          {item.detail[0].pax.length} orang
        </Text>
      );
    ...
    if (item.product == "Flight") {
      ...
      schedule = (
        <View style={{ flex: 1, flexDirection: "row" }}>
          ...
          >
            <Text caption2 style={{ color: BaseColor.redColor }}>
              Start Date :{/* {item.detail[0].order.checkin} */}
            </Text>
          </View>
          <View
            style={{
    ...
    total_price = (
      <Text title3 numberOfLines={1} style={{ color: BaseColor.primaryColor }}>
        Rp {priceSplitter(item.total_price)}
      </Text>
    );

    if (order_payment != null) {
      var expiredTime = this.duration(order_payment.expired);
      ...
      no_tagihan = (
        ...
        >
          No.Tagihan {order_payment.id_invoice}
        </Text>
      );
      ...
      masa_tagihan = (
        <View style={{ flex: 1, flexDirection: "row" }}>
          ...
          <View style={styles.rightSub}>{countDown}</View>
        </View>
      );

      if (
        order_payment.payment_type == "" ||
        order_payment.payment_type == "user"
      ) {
        var dataPayment = {};
        ...
        var dataPayment = {
          ...
          payment_type_label: order_payment.payment_type_label,
          payment_sub: order_payment.payment_sub,
          payment_sub_label: order_payment.payment_sub_label,
        };
        var urlRedirect = "Pembayaran";
      }
    ...
    var urlImage =
      "https://masterdiskon.com/assets/upload/product/hotelpackage/2020/4be994f3bf41842cbef6626c815d18a5.jpg";
    if (loading == true) {
      content = (
        ...
        >
          ...
          {item.product == "Flight" ? (
            ...
            >
              <View>
                <Text caption1>
                  {item.product_detail.airline_name} (
                  {item.product_detail.airline_code})
                </Text>
              </View>
              <View>
                <Text caption1>
                  {item.detail[0].order_detail[0].cabin_name}
                </Text>
              </View>
            </View>
          ) : (
          ...
          {item.product == "Flight" ? (
            ...
            >
              <View>
                <Text caption1 style={{ color: BaseColor.blackColor }}>
                  Waktu Berangkat :{" "}
                  {this.convertDateText(
                    item.detail[0].order_detail[0].departure_date
                  )}
                  , {item.detail[0].order_detail[0].departure_time}
                </Text>
              </View>

              <View>
                <Text caption1 style={{ color: BaseColor.blackColor }}>
                  Pax / Guest : {item.detail[0].pax.length}
                </Text>
              </View>
            </View>
          ) : (
